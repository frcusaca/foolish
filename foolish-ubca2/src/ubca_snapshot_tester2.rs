...
#[cfg(test)]
mod temporary_foop86_answer_comparison {
    use std::path::{Path, PathBuf};
    ...
    fn walk(dir: &Path, root: &Path, out: &mut Vec<PathBuf>) {
        ...
        for e in rd.flatten() {
            ...
            if p.is_dir() {
                walk(&p, root, out);
            } else if p.extension().is_some_and(|x| x == "foo") {
                out.push(p.strip_prefix(root).unwrap().to_path_buf());
            }
        }
    }
    ...
    #[test]
    fn temporary_foop86_do_the_two_evaluators_agree() {
        let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let ubca_input_root = manifest.join("../foolish-ubca/einmo_suite/input");
        let ubca_checked_root = manifest.join("../foolish-ubca/einmo_suite/checked");
        let ubca2_checked_root = manifest.join("einmo_suite2/checked");

        let mut rels = Vec::new();
        walk(&ubca_input_root, &ubca_input_root, &mut rels);
        rels.sort();

        let mut differing = Vec::new();
        let mut missing_ubca = Vec::new();
        let mut missing_ubca2 = Vec::new();
        for rel in &rels {
            let ubca_path = ubca_checked_root.join(rel).with_extension("foo.einmo");
            let ubca2_path = ubca2_checked_root.join(rel).with_extension("foo.einmo");
            let Some(ubca_out) = output_of(&ubca_path) else {
                missing_ubca.push(rel.clone());
                continue;
            };
            let Some(ubca2_out) = output_of(&ubca2_path) else {
                missing_ubca2.push(rel.clone());
                continue;
            };
            if normalize(&ubca_out) != normalize(&ubca2_out) {
                differing.push((rel.clone(), ubca_out, ubca2_out));
            }
        }

        eprintln!(
            "=== FOOP-86 Phase 2: {} shared inputs, {} differing (rendering or value), {} missing-ubca-checked, {} missing-ubca2-checked ===",
            rels.len(),
            differing.len(),
            missing_ubca.len(),
            missing_ubca2.len(),
        );
        for m in &missing_ubca {
            eprintln!("MISSING UBCA CHECKED: {}", m.display());
        }
        for m in &missing_ubca2 {
            eprintln!("MISSING UBCA2 CHECKED: {}", m.display());
        }

        // Strip the old renderer's FIR-internal "machinery" groups —
        // `?(pattern='^x$', UNANCHORED, ECONSTANIC)` — before comparing
        // digits, mirroring t12_value_diff's technique, so that a case
        // which differs ONLY in how the same answer is described (Foolish
        // sugar vs FIR-internal detail) is not misreported as a semantic
        // disagreement.
        fn strip_machinery(s: &str) -> String {
            let mut out = String::new();
            let mut depth = 0usize;
            let mut prev = ' ';
            for ch in s.chars() {
        ...
        let mut real_value_diff = Vec::new();
        for (rel, old, new) in &differing {
            let o_clean = strip_machinery(&normalize(old));
            let n_clean = normalize(new);
            let od: String = o_clean.chars().filter(|c| c.is_ascii_digit()).collect();
            let nd: String = n_clean.chars().filter(|c| c.is_ascii_digit()).collect();
            if od != nd {
                real_value_diff.push((rel.clone(), old.clone(), new.clone(), od, nd));
            }
        }

        eprintln!(
            "=== FOOP-86 Phase 2: of those, {} are DIGIT-LEVEL value differences (candidates for a real disagreement) ===",
            real_value_diff.len(),
        );
        for (rel, old, new, od, nd) in &real_value_diff {
            eprintln!("--- DIGIT DIFF: {} (ubca digits [{od}] vs ubca2 digits [{nd}]) ---", rel.display());
            eprintln!("UBCA:\n{old}");
            eprintln!("UBCA2:\n{new}");
        }
    }
