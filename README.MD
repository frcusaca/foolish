...
No complex import systems, no verbose inheritance hierarchies—just natural, 
...
on computers—it bridges human and computational intelligence.
...
have observably limited size. Cells can only be so big before the the split or
death. Atomic and subatomic objects has to be so close or otherwise the forces that keep
them together stops working. Therefore the Foolish brane is also limited in size.
The Foolish brane certainly should have finite size, and depending on the computer
it may have a specified limit in number of entries.

Inside the brane, it is a one dimentional object. Its entries line up one after another.
The true dimension of entries are computational dependencies. So depending on the code
in side, the actual dependency axis could have smaller dimensions best visualized
as a dependency DAG. The dependency DAG and its subdimensions branch out and progress
forward in their own time dimensions. Sometimes more than one timeline merge together
by being involved in an expression that refers to all of them. Overall, the brane's
facturing timelines can be linearized, merging into the sequence of code as they originally
...
One will note that it didn't matter how evaluation occurred—either `(q p) f` 
...
collection—there is meaning in it and there is meaning in their collection 
...
[Relational Coordinates](#relational-coordinates) section for expanded discussions.
...
in an unanchored search: `↓`, `#@+1`, `→`, `#+1`. However, if an existing 
...
For now we forbid the use of unanchored `#0` searching to beginning of the current line.
The only permitted recursive call is the `↑` operator. An expression that contains 
recursion must be an expression with a brane that refer's to the line in the parent brane
...
context decontexualizing x and y. x and y become unbound symbols. From timeline
perspctive, the detachment snips off dependency DAG edges.
...
When default branes are next to each other, they are right associative—the 
...
is always right-associative—it is applied to the brane on the right side before
...
```foolish
{
    ...
    result = condition⇒;                !! shorthand for `/f'condition:true→` !! That uses a double-lined arrow in '=⇒' which is different from next statement single-arrow
...
finding the current brane is easy with `↑`. So recursion is easy to express:
```foolish
{
    factorial = [n=???]{
        result = if n <= 1 
...
inside the unicellular brane computer so far, not including name of the current expression.
...
In some cases, such as the recursive call to `↑`, the search finds a line of code that
has incomplete value. Note by incomplete value, in the UBC, we mean not the unkown 
...
expression using the context "IB AB ↑". That says the UBC shall evaluate the AST of the 
expression that created the present brane, resolving any searches
...
In that example, in recursive call to compute r is processed by the UCB to the
equivalent of:
```foolish
   {
      ...
...
to AST and stored in memory by the time UCB is requied to evaluate this line of
Foolish. Therefore in constant time, the uncoordinated ↑ is translated into an
AST expression. This AST expression may further contain ↑ inside or at its front.
But that is for a later iteration of UCB to evaluate. Each step taken by the UCB
...
3. A comment.
4. An abstract brane in that it is deliberately or inadvertently undercontextualized--
     ...
     possible. That brane with all possible values coordinated is a fully evaluted
     abstract brane. Normally, a recursive brane conditions on some computational result
     that depends on a parameter--a coordinate identifier that is named but unattached
     ...
     such a brane will be finite or infinite indepth depending on its construction.
     the abstract brane may therefore be permenantly underevaluated due to its construction.
     An abstract brane is fully evaluated if the underevaluated members are due to detached
     identifier names.
4. An underevaluated FIR represents code that was expressed in Foolish, but it 
   has not been fully evaluated in the present UBC context. This FIR contains the AST of
   the expression, and a reference to the AB and IB that was used to attempt
...
3. If brane concatenation is involved, then process the branes based on precedence
   rules: detachment brane associating right using First-Right-Left-Last(FRLL), then brane
...
2. Each line of FIR is evaluated using expression evaluation.
   1. if the FIR is a value, it is maintained as value.
   2. if the FIR is an expression, it is evaluated, the new FIR retained. (*)
   2. IB is updated.
   3. proceed to next line.
4. When the brane ends, the brane is returned as a value.

(*) The expression is underevaluated if it makes an underevaluated references.
When the underevaluation is caused by `↑` recursion this single step materializes one
step of the recursion. The FIR is updated to replace `↑` with a FIR representing 
`[IB, AB] a`--the `a` being the AST of `↑`. This concludes the iteration.
Note, the next time we take a step in UBC pass , this FIR is a brane concatenation that
...
The UBC FIR processing has following stages:
1. AST
...
### Typing
Types have become very central to humanity's aporoach to organize and understand
...
characterizations are Foolish programs that establishes whether a characterizable has
or does not have said characterization. These are the most inclusive and the most exclusive
characterization available in Foolish.
...
x and y coordintes in a two dimensional cartesian graph. But in reality, almost all
...
or `{a,b,c} is_median_height`, which may return boolean whether `a.x`, `b.x`, `c.x` is
sorted, or `a` is median of `b` and `c`, respectively.

So, in Foolish, a value that is accessible *after* a brane is fully evaluated is considered 
a coordinate--the value tells us how to relate to the brane that was just comptued. A well 
coordinated brane is one that is fully evaluated to values so that computation can procede
 ...
 * Homorphic Encryption and equivalent "thought block" builtin.
 * Asynchornized execution.
 * Realtime computations.
 * Builtin masking of data from compute.
 * Library ecosystem for interactivity with python/java/c++/rust/go/etc.Other 
   ...
   spark, arrow, standards such as JSON, XML, HTM, PPML, ONNX, protocol buffers, etc.
## Development Notes
