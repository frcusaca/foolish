...
mod tests {
    ...
    fn test_identification_and_use() {
        ...
        assert_eq!(result.statements[0].state, Nyes::Independent);
        // y resolves to x which is Independent, so y is also Independent
    ...
    // Cross-validation: UBCb produces same result as UBC
    #[test]
    ...
    fn cross_validate(source: &str) {
        ...
        // Compare statement states using UBCb's evaluation result (already stepped)
