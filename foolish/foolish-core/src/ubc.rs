...
use crate::fir::{
    clone_steppable, fir_to_ref, Fir, FirRef, NormalBraneFir, Nyes, StatementFir, Steppable,
};
...
impl Scope {
    ...
    pub fn block_brane_searches(&self) -> bool {
        self.block_brane_searches
    }

    pub fn set_block_brane_searches(&mut self, v: bool) {
        self.block_brane_searches = v;
    }
    ...
    pub fn current_stmt_idx(&self) -> Option<usize> {
    ...
    match fir.borrow().fir_variant() {
        "NormalBrane" => {
            ...
            if let Fir::NormalBrane(inner) = f {
                inner.statements.iter().any(|s| {
                    s.state == Nyes::Econstanic || has_unresolved_forward_refs_in_fir(&s.body)
                })
            } else {
                false
            }
        ...
        "Operator" => {
            ...
            if let Fir::Operator(inner) = f {
                inner
                    .operands
                    .iter()
                    .any(has_unresolved_forward_refs_in_fir)
            } else {
                false
            }
        }
...
pub fn re_step_brane_bodies(brane: &mut NormalBraneFir, scope: &Scope) -> Result<(), UbcError> {
    ...
    let statements: Vec<StatementFir> = statements
        .into_iter()
        .map(|s| {
    ...
    for (idx, stmt) in statements.iter().enumerate() {
        let scoped = local_scope.clone().with_brane(
            fir_to_ref(Fir::NormalBrane(Box::new(NormalBraneFir {
        ...
        if let Some(ref name) = stmt.name {
            local_scope.push(name.clone(), fir_to_ref(body.clone()));
        }
...
fn reset_searches(fir: Fir) -> Fir {
    match fir {
        Fir::Search(inner) => {
            ...
            s.anchor = s
                .anchor
                .map(|a| fir_to_ref(reset_searches(clone_steppable(&a))));
        ...
        Fir::Operator(inner) => {
            ...
            op.operands = op
                .operands
                .into_iter()
                .map(|e| fir_to_ref(reset_searches(clone_steppable(&e))))
                .collect();
        ...
        Fir::Concatenation(inner) => {
            ...
            c.merged = c
                .merged
                .map(|m| fir_to_ref(reset_searches(clone_steppable(&m))));
...
fn extract_variant(fir: &FirRef) -> Variant {
    ...
    match fir_clone {
        ...
        Fir::Operator(inner) => Variant::Operator(
            inner.op,
            inner.operands.iter().map(|o| clone_steppable(o)).collect(),
        ),
...
pub fn short_circuit(fir: &FirRef) {
    ...
    if let Some(end) = end_target {
        fir.borrow_mut()
            .set_search_target_direct(Some(Rc::clone(&end)));
    }
}
...
pub fn compute_brane_state(statements: &[StatementFir]) -> Nyes {
    ...
    if statements
        .iter()
        .all(|s| matches!(s.state, Nyes::Constant | Nyes::Independent))
    {
        Nyes::Constant
        ...
        .iter()
        .any(|s| matches!(s.state, Nyes::Econstanic | Nyes::Woconstanic))
    {
        Nyes::Woconstanic
    } else {
        Nyes::Braning
