...
- **Ecosystem**: Library design, interop with native/FFI ecosystem, package management
...
Foolish is implemented in Rust using:
- Cargo/rustc for builds
- Insta (snapshot) approval tests for verifying compiler/runtime output
- A Cargo workspace with crates including `foolish-core`, `foolish-ubcb`, and `foolish-cli`
