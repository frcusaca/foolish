...
> **Clarified by FOOP-55 §8 (2026-08-11).** Rule 4's first sub-case — different
> non-NK constanic kinds — is there named **"not mutually identifiable"**: a
> number and a brane can never bear the same identity, so the pair is *decided*,
> not undecided. That is why it is `NotEqual` and not `Unknowable`. **The
> behaviour specified here is unchanged**; FOOP-55 only supplies vocabulary and
> makes the choice an explicit constant
> (`NOT_MUTUALLY_IDENTIFIABLE_IS_NOT_EQUAL`, `foolish-ubca/src/fir_kinds.rs`).
> A future FOOP should restate the equality primitive succinctly in one place.
