...
| [FOOP-55](FOOP-55.md) | Project Euler 1 — 'mod, 'or, and the repairs that run the first exercise | Implementing | phase-4 | 2026-08-07 | Sisyphus / qwen3.8-max |
...
- [FOOP-55](FOOP-55.md) — Project Euler 1: make the first exercise run — `'mod` integer modulo (FOOP-33 §5.1 BodyOverride mechanism, integer result), `'or` boolean OR (FOOP-73 preferred pure-Foolish truth-table design, `'or` only), plus documented platform defects D1–D6 (incl. the leading-`_` lexer workaround via `INTERN_` prefix and the `$=`/`=$` sugar findings, Atlas-directed) and exercise defects E1–E5. **Revised 2026-08-10: adds §5, the upgraded SFF mark** — `constanic_clone_at` currently strips EVERY SF/SFF mark in one recursive pass, so a mark protects a term across exactly one coordination; §5 makes it a counter (at most one strip per clone TREE, retained marks shared by `Rc::clone`). A correctness fix: premature stripping resolves against the WRONG neighbours and an early miss settles NK, which is terminal. It is also what makes the exercise terminate — `'ite`'s doubly-marked branches survive table construction, so the unselected branch is never coordinated again and `<loop>` never recurses. No FVM stepping change, no new FIR kind, no new NYES state. **Appendix A** records the four designs considered and authoritatively defines the UBC code names (UBCa reference / UBCb dependency-tracking, attempted / UBCc breadth-first / UBCd message passing). **FOOP-65 dependency DROPPED** (2026-08-09): verified live that the exercise's juxtaposition form evaluates as written, so the backtick rewrite is optional polish, not a precondition
...
- **[FOOP-65](FOOP-65.md)** — **the tail concatenator** (backtick). `fn`{p1,p2}` ≡
  ...
  constituents are embryonic. ~~Prerequisite of FOOP-55~~ (dropped 2026-08-09); **depends on FOOP-95** for the vantage
