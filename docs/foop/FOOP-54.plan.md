...
- [x] Complete. FOOP-54 was one arm of a two-arm implementation-performance comparison
      (mimo-opencode built this arm; Claude Opus 4.8 built the identical FOOP-92). The comparison
      concluded: FOOP-92 was judged the better implementation (see FOOP-54.md §"Post
      implementation comparison"), was improved to mergeable quality, and merged to `jia`
      (9bbdaf43). This arm's implementation is retired; the remaining checkboxes below are
      cancelled `[-]` — they were either fulfilled by the merged FOOP-92 arm or are moot. The
      `.snap`-corpus migration in Deferred is now owned by FOOP-64.
      (2026-07-14 10:14)
- [x] Integrate the lessons learned from the comparison (FOOP-54.md §9 "Best Practices Review")
      into `rust_instructions.md` — visibility via private modules + selective `pub use`, one
      centralized thiserror enum per library, Vec-not-HashMap for ordered data, enums over
      stringly-typed finite domains, closure-injected side effects for testability, no dead-code
      stubs, real provenance hashes, explicit Argon2id parameter pinning, byte-exact parsing under
      signatures, `ExitCode` over `process::exit`, hermetic tests that never modify repo state.
      **This was the only open item left for FOOP-54.** All fifteen §9 recommendations were
      imported with short examples (cited c25–c28), and a new "§2 Task guides" section re-indexes
      the rules by task, most-frequent first (function → helper → naming → documentation →
      module → crate). FOOP-54 is now fully closed.
      (2026-07-14 10:14)
...
**Changes**: Marked the plan **Complete** (`[x]` at top): FOOP-54 concluded as the mimo-opencode
arm of the two-arm implementation comparison against FOOP-92 (Claude Opus 4.8); FOOP-92 won, was
hardened, and merged (9bbdaf43). All remaining checkboxes cancelled `[-]` per foop.md convention.
One open item added (and it is the only one): integrate the §9 Best Practices Review lessons into
`rust_instructions.md`. Noted that the Deferred `.snap`-corpus migration is now owned by FOOP-64.
