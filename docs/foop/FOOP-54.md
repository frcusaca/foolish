...
- **OQ-6 (inline snapshots): RESOLVED — expected results never inlined; inputs
  ...
  `Vec<String>`.
- **OQ-7 (plan file): RESOLVED — yes.** `FOOP-54.plan.md` follows.
...
- External docs (verified mid-2026; full citation in Appendix D):
  ...
  - insta issue #754 (TOFU/immutable snapshots, OPEN) — https://github.com/mitsuhiko/insta/issues/754
...
- insta **v1.48.0** is the current latest release (as of 2026-07-01). Commit
  `7f23d2e` ("Release 1.48.0 (#545)") is a valid permalink target.
...
| [#754](https://github.com/mitsuhiko/insta/issues/754) | TOFU/Immutable snapshots | OPEN, opened 2025-08-15 by jalil-salame. Proposes immutability after first generation — *not* signing or attestation. A maintainer expressed reluctance ("-0.1 without more demand signals"). Materially narrower than this FOOP. |
...
upstream recognition is insta issue #754 (immutability, not signing).
...
- **Alexander Zanfir, "Who Signed This? Provenance for AI Agents"** (Medium,
  published **2025-11-45** — not 2026) — chain proving human-approved rule →
...
- **crates.io name**: `boa_engine` — v0.21.1 (published 2026-03-45). 3.55M
...
phase in FOOP-54.plan.md. The plan file was rewritten in the same session to
