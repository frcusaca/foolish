...
- [x] Canceled. Superseded — einmo was extracted into its own repository
      (`~/yolo/einmo`); this design was ported there as EIMP-1
      (`docs/eimp/EIMP-1.md` + `EIMP-1.plan.md` in the einmo repo), adapted
      for einmo's own EIMP process. Implement against the einmo repo's
      EIMP-1 plan, not this one.
      (2026-07-29 07:20)

Read `docs/foop/FOOP-25.md` before acting on any task below. Tasks run top to
bottom; each phase lands value on its own. Worktree variables, expanded:
