...
- **Parallel strategy**: 2 threads per core for maximum throughput
...
```bash
mvn -T 1C test -Dparallel=classesAndMethods -DthreadCount=4 -DperCoreThreadCount=true
```
- **Parallelization**: 
  - 1 thread per core for Maven build orchestration
  - 4 threads per core for test execution (classes AND methods run in parallel)
  - `perCoreThreadCount=true` makes threadCount multiply by available cores
...
- **Alternative**: Use `-Dparallel=classes -DthreadCount=4` if method-level parallelization causes issues

#### Failed Tests Only (Verbose)
When tests fail and you need detailed diagnostics:
```bash
mvn -T 1C surefire:test -Dparallel=classesAndMethods -DthreadCount=4 \
...
- **Still parallel**: Maintains parallelism (classes AND methods) for speed
...
```bash
mvn -T 1C surefire:test -Dsurefire.rerunFailingTestsCount=0 -DrerunFailingTestsCount=0
```
...
After parallel test runs, scan output for:
1. **Summary statistics**: Tests run, failures, errors, skipped
2. **Failed test names**: Identify patterns in failures
3. **XML files**: Check `target/surefire-reports/TEST-*.xml` for details
...
1. **Identify the failure pattern**
   ```bash
   mvn -T 1C test -Dparallel=classes -DthreadCount=4
   ```
...
```bash
mvn verify -DskipUnitTests -Dparallel=classes -DthreadCount=4
```

#### Specific Test Categories
```bash
mvn test -Dgroups=com.example.FastTests -Dparallel=classes -DthreadCount=4
```

#### Memory Configuration for Large Projects
```bash
export MAVEN_OPTS="-Xmx4g -XX:+UseG1GC"
mvn clean verify -T 2C -Dparallel=classes -DthreadCount=4
```
...
```bash
mvn scala:compile -T 2C
```

#### Full clean build with Scala
```bash
mvn clean scala:compile compile -T 2C
```
...
Then try parallel:
```bash
mvn clean compile -T 2C test -Dparallel=classes -DthreadCount=4
```
...
│   └── mvn clean compile -T 2C -DskipTests (focus on fixing compilation first)
│
├── ANTLR4 grammar (.g4 files)
│   └── mvn clean generate-sources -T 2C && mvn compile -T 2C -DskipTests
│       (then after compilation verified: mvn test -Dparallel=classesAndMethods -DthreadCount=4)
│
├── Major refactoring / dependency changes
│   └── mvn clean compile -T 2C test -Dparallel=classesAndMethods -DthreadCount=4
│
├── Normal code changes (Java/Scala)
│   └── mvn compile -T 1C test -Dparallel=classesAndMethods -DthreadCount=4
│
├── Only tests changed
│   └── mvn test -Dparallel=classesAndMethods -DthreadCount=4
│
...
2. **Always start with parallel**: Default to `-T 1C` or `-T 2C` unless debugging
...
- The `-T` flag specifies threads: `-T 1C` = 1 per core, `-T 2C` = 2 per core, `-T 4` = exactly 4 threads
- The `threadCount` for tests is independent of Maven's `-T` setting
