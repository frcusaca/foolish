...
public class BraneMemory implements BraneMemoryInterface<BraneMemory.Query, FIR> {
    public sealed interface Query permits IdentifierQuery, RegExpQuery {}

    public final class IdentifierQuery extends CharacterizedIdentifier implements Query {
        public IdentifierQuery(String name, String characterization) {
            super(name, characterization);
        }
    }

    public record RegExpQuery(Pattern pattern) implements Query {
