...
public final class FinearVmSimple implements FinearVmAbstract {
    ...
    public Firoe evaluate(Firoe firoe, Env env) {
        ...
        if (firoe instanceof ProgramFiroe pm) {
            ...
            if (l instanceof Firoe lm && lm instanceof Finear flm && !flm.isNK()
                    && r instanceof Firoe rm && rm instanceof Finear frm && !frm.isNK()) {
            ...
            if (res instanceof Firoe mres && !mres.isNK() &&
            ...
            switch (eval_cond(im.condition(), im.thenExpr(), env)) {
                ...
                case Finear f when f == Finear.NK -> {
                }
                case Firoe m -> {
                    result = m;
                }
        ...
        } else if (firoe.base() instanceof Insoe(AST ast) && ast instanceof AST.UnknownExpr) {
            result = Finear.NK;
        } else {
    ...
    // if condition is not knowable, then NK is returned without executing then branch
